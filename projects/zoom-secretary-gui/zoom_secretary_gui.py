...
class App:
    def __init__(self, root):
        ...
        if not self.api_key:
            messagebox.showerror("오류", "Gemini API Key가 설정되지 않았습니다.\n.env 파일 또는 환경 변수를 확인하세요.")
            sys.exit(1)
    ...
    def setup_ui(self):
        ...
        # Header Title
        title_label = tk.Label(main_frame, text="🎙️ AI Zoom 수업 비서", font=("맑은 고딕", 15, "bold"), fg="#1f4e79", bg="#f8f9fa")
    ...
    def toggle_recording(self):
        if not self.is_active:
            ...
            self.status_var.set("🔴 수업을 들으며 녹음 및 화면 캡처 중...")
    ...
    def cancel_recording(self):
        ...
        self.action_btn.pack_forget()
        self.action_btn.pack(fill=tk.X)
        self.action_btn.config(state="normal", text="수업 시작 (50분 녹음)", bg="#1f4e79", activebackground="#153654")
    ...
    def stop_and_process(self):
        ...
        # Revert button layouts to single status button
        self.pause_btn.pack_forget()
        self.cancel_btn.pack_forget()
        self.action_btn.pack_forget()
        self.action_btn.pack(fill=tk.X)
    ...
    def process_audio_and_generate_summary(self):
        ...
        if audio_data is None or len(audio_data) == 0:
            self.root.after(0, lambda: self.reset_ui_error("녹음된 오디오 데이터가 없습니다.\n\n[체크포인트]\n1. 선택한 사운드 장치(스피커/헤드폰)로 실제로 수업 소리가 나고 있었는지 확인하세요.\n2. 마이크 장치가 아닌 '[시스템 소리 녹음]'이라고 표시된 장치를 선택하세요."))
            return

        # Step 1: Resampling Audio (0% -> 25%)
        ...
        self.root.after(0, lambda: self.update_gui_progress(25.0, "🎵 로컬 WAV 오디오 파일 저장 완료."))
        time.sleep(0.5)

        # Step 2: Uploading to Gemini (25% -> 50%)
        self.root.after(0, lambda: self.update_gui_progress(35.0, "☁️ 구글 제미나이 서버로 오디오 업로드 중..."))
        ...
        try:
            ...
            # Gemini API(httpx)는 HTTP 헤더에 ASCII만 허용하므로 한글 경로/파일명 전달 시 UnicodeEncodeError 발생.
            # 파일 객체와 UploadFileConfig(display_name, mime_type)를 함께 사용해 우회.
            ...
            self.root.after(0, lambda: self.update_gui_progress(50.0, f"☁️ 업로드 완료 (ID: {audio_file.name})"))
            time.sleep(0.5)

            # Step 3: Polling Gemini server processing (50% -> 75%)
            self.root.after(0, lambda: self.update_gui_progress(55.0, "⚙️ 제미나이 서버의 오디오 인덱싱 대기 중..."))
            
            wait_count = 0
            # Poll using the new SDK syntax
            while "PROCESSING" in str(audio_file.state):
                wait_count += 1
                self.root.after(0, lambda w=wait_count: self.status_var.set(f"⚙️ 제미나이 오디오 분석 및 변환 대기 중... ({w}초 경과)"))
                time.sleep(1)
                audio_file = client.files.get(name=audio_file.name)
                
            if "FAILED" in str(audio_file.state):
                raise Exception("제미나이 서버 오디오 분석에 실패했습니다.")

            self.root.after(0, lambda: self.update_gui_progress(75.0, "⚙️ 오디오 인덱싱 완료. 요약 생성 준비 중."))
            time.sleep(0.5)

            # Step 4: Generating Multimodal Content Note (75% -> 95%)
            self.root.after(0, lambda: self.update_gui_progress(80.0, "📝 AI 멀티모달 요약본 생성 및 학습 노트 작성 중..."))

            # Create Gemini contents using new SDK
            contents = [audio_file]
            contents.append("\n\n아래는 강의 동안 타임스탬프 순서대로 캡처된 컴퓨터 화면 이미지들입니다:\n")
            
            # Read files from local disk to save RAM
            for shot in screenshots:
                contents.append(f"\n[화면 캡처 타임스탬프: {shot['timestamp']}]\n")
                if os.path.exists(shot['file_path']):
                    img = Image.open(shot['file_path'])
                    contents.append(img)
            ...
            # Call generate_content on the new SDK with gemini-2.5-flash
            ...
            # Clean up on Gemini server using new SDK
    ...
    def reset_ui_success(self, md_path):
        ...
        self.progress_bar.pack_forget() # Hide when done
    ...
    def reset_ui_error(self, err_msg):
        ...
        self.monitor_combo.config(state="readonly")
        self.audio_combo.config(state="readonly")
        self.progress_bar.pack_forget() # Hide on error
